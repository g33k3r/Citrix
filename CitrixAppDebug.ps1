...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="Citrix App Migration Tool" Height="750" Width="800">
    <Grid>
        <Grid.RowDefinitions>
        ...
        <StackPanel Grid.Row="2" Orientation="Horizontal" Margin="10">
            <Label x:Name="ApplicationGroupsLabel" Content="Destination App Groups:" Width="150"/>
        ...
        <TextBox x:Name="StatusTextBox" Grid.Row="7" Grid.ColumnSpan="2" Height="100" Width="760" Margin="10" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto" IsReadOnly="True" TextWrapping="Wrap"/>
    </Grid>
</Window>
"@
...
try {
    ...
    $adminFolderListBox = $window.FindName("AdminFolderList")
    ...
    if (-not $window -or -not $sourceControllerBox -or -not $destinationControllerBox -or -not $applicationGroupListBox -or -not $adminFolderListBox -or -not $applicationGroupsLabel -or -not $userFolderBox -or -not $queryButton -or -not $copyButton -or -not $applicationsDataGrid -or -not $statusTextBox) {
        throw "One or more UI elements could not be found. Please check the XAML structure."
    }
    ...
    $copyButton.Add_Click({
        try {
            ...
            # Get the selected admin folder
            $selectedAdminFolderName = $adminFolderListBox.SelectedItem
            ...
            # Retrieve the admin folder object from the destination controller
            $selectedAdminFolder = $null
            if (-not [string]::IsNullOrEmpty($selectedAdminFolderName)) {
                $selectedAdminFolder = Get-BrokerAdminFolder -AdminAddress $destinationController -Name $selectedAdminFolderName

                if ($null -eq $selectedAdminFolder) {
                    [System.Windows.MessageBox]::Show("Selected admin folder not found on the destination controller.")
                    Add-Status "Selected admin folder not found on the destination controller."
                    return
                }
            }
            ...
            try {
                foreach ($selectedApp in $selectedApplications) {
                    try {
                        ...
                        if ($null -ne $selectedAdminFolder) {
                            $params["AdminFolderUid"] = $selectedAdminFolder.Uid
                        }
            ...
            [System.Windows.MessageBox]::Show("An error occurred while copying applications: $_")
            Add-Status "An error occurred while copying applications: $_"
        }
    })
